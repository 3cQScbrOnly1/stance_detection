...
#include "NNCharBiLSTMDetector.h"
...
void Detector::createAlphabet(const vector<Instance>& train_instances)
{
	...
	else
	{
		...
		string cur_word, cur_char;
		int target_size, tweet_size, instance_size, target_word_size, tweet_word_size;
		instance_size = train_instances.size();
		for (int i = 0; i < instance_size; i++)
		{
			
			m_driver._model_params._label_alpha.from_string(train_instances[i].m_stance);
			target_size = train_instances[i].m_target.size();
			for (int j = 0; j < target_size; j++)
			{
				cur_word = normalize_to_lowerwithdigit(train_instances[i].m_target[j]);
				m_word_stats[cur_word]++;
				target_word_size = train_instances[i].m_target[j].size();
				for (int k = 0; k < target_word_size; k++)
				{
					cur_char = train_instances[i].m_target[j][k];
					m_char_stats[cur_char]++;
				}
			}
			tweet_size = train_instances[i].m_tweet.size();
			for (int j = 0; j < tweet_size; j++)
			{
				cur_word = normalize_to_lowerwithdigit(train_instances[i].m_tweet[j]);
				m_word_stats[cur_word]++;
				tweet_word_size = train_instances[i].m_tweet[j].size();
				for (int k = 0; k < tweet_word_size; k++)
				{
					cur_char = train_instances[i].m_tweet[j][k];
					m_char_stats[cur_char]++;
				}
			}
		}
...
void Detector::train(const string& trainFile, const string& devFile, const string& testFile, const string& modelFile, const string& optionFile)
{
	...
	if (m_options.charFile != "")
		m_driver._model_params._chars.initial(&m_driver._model_params._char_alpha, m_options.charFile, m_options.charEmbFineTune);
	else
		m_driver._model_params._chars.initial(&m_driver._model_params._char_alpha, m_options.charEmbSize, m_options.charEmbFineTune);
